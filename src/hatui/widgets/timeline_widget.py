...
class TimelineWidget(Widget):
    ...
    @property
    def _schema(self):
        return {"events_key": str, "fg_color": str, "bg_color": str, "show_time": bool}
    ...
    def paint(self, buffer, context: WidgetContext):
        ...
        if rect.width <= 0 or rect.height <= 0 or not events:
            return
        ...
        for row_index, event in enumerate(visible):
            ...
            marker_color = resolve_color_token(
                context.theme.widget_slot("timeline", f"{level}_fg_color", context.theme.color(level, base_style.fg_color)),
                context.theme,
                base_style.fg_color,
            )
